...
// 메시지 출력을 위한 알려주는 변수. extern으로 선언 시 선언된 변수가 어딘가에
// 있다는 것을 아두이노 소프트웨어에 알릴 때 사용//
...
void calibAccelGyro() {
    ...
    // 초기 보정값은 10번의 가속도 자이로 센서의 값을 받아 해당 평균값을
    // 가진다.//
    for (int i = 0; i < 10; i++) {
        readAccelGyro();
...
////////////////////////
void initYPR() {
    // 초기 호버링의 각도를 잡아주기 위해서 Roll, Pitch, Yaw 상보필터 구하는
    // 과정을 10번 반복한다.//
    for (int i = 0; i < 10; i++) {
        readAccelGyro();
        calcDT();
...
void loop() {
    ...
    if (tmp == 100) {
        ...
        Serial.print("motorD speed : ");
        Serial.println(motorD_speed);
        Serial.print("\n");
        tmp = 0;
    }
}
...
void calcAccelYPR() {
    ...
    accel_angle_z = 0; // 중력 가속도(g)의 방향과 정반대의 방향을 가리키므로
...
void updateMotorSpeed() {
    ...
    analogWrite(3, motorA_speed);
    analogWrite(9, motorB_speed);
    analogWrite(10, motorC_speed);
    analogWrite(11, motorD_speed);
}