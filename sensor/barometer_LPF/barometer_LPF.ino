...
float low_pass_filter(float pressure) {
  ...
  // Low Pass Filter 방정식
  LPF_filtered_x = ((LPF_lambda / (1 + LPF_lambda)) * LPF_x)
                   + ((1 / (1 + LPF_lambda)) * LPF_prev_filtered_x);
...
float filter2(float pressure) {
  filtered2_pressure = filtered2_pressure + (((pressure + prev_filtered2) / 2.0) - filtered2_pressure) * 0.5;
